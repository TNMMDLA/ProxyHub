...
Security summaries expose configured booleans and counts only: authentication, 2FA availability, secure-cookie mode, trusted proxy, rate limiting, audit logging, and encryption configuration. They do not expose keys, token hashes, usernames, IPs, user agents, session details, or audit metadata.
